...
interface CodeScanChainContainerProps {
  title: string;
  icon?: React.ReactNode;
  steps: CodeScanStepProps[];
  className?: string;
  defaultExpanded?: boolean;
}

function CodeScanStep({
  title,
  description,
  content,
...
export function CodeScanChainContainer({
  title,
  icon,
  steps,
  className,
  defaultExpanded = false,
}: CodeScanChainContainerProps) {
  const [isExpanded, setIsExpanded] = useState(defaultExpanded);
          ...
          <div className="space-y-4 pt-6">
            {steps.map((step) => (
              <CodeScanStep
                ...
                defaultExpanded={false} // All steps collapsed by default
              />
            ))}
          </div>
        </div>
